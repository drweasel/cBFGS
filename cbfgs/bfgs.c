...
/* work: n values */
static bool
LDLt_factor(double* A, const int n, double* work)
{
    bool result = true;
    double* v = work;
    for (int j = 0; j < n; ++j)
...
/* work: n values */
static void
LDLt_solve(const double* LD, const int n, double* b, double* work)
{
    double* zy = work;
    /* Forward; solve L.z = b */
    for (int i = 0; i < n; ++i)
...
dot_v(const double* a, const double* b, const unsigned int n)
{
    double d = 0.;
...
static inline double
norm_v(const double* v, const unsigned int n)
{
  ...
  double* work)
{
    ...
    for (;;)
    {
        ...
        if (
          phi_alpha > phi_0 + c1 * alpha * dphi_0 ||
