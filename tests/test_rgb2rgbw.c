...
RGB KtoRGB(double T, double Y)
{
	return XYZtoRGB(KtoXYZ(T,Y));
}

RGB_Decomp new_rgb_decomp(
	RGB obj_rgb,
	RGB* base,
	int dim
	)
{
	...
	dcmp.base = (RGB*)malloc(dim*sizeof(RGB));
	for (int k=0; k<dim; ++k)
		dcmp.base[k] = base[k];

	dcmp.s = (double*)malloc(dim*sizeof(double));
	for (int k=0; k<dim; ++k)
		dcmp.s[k] = 0.5;
...
void free_rgb_decomp(RGB_Decomp* dcmp)
{
	free(dcmp->base);
	free(dcmp->s);
}

double rgb_decomp_obj_f(const double* s, void* uptr)
{
...
void rgb_decomp_obj_Df(const double* s, double* Df, void* uptr)
{
	RGB_Decomp* dcmp = (RGB_Decomp*)uptr;
	RGB e;
	e.R = e.G = e.B = 0.;
...
bool decompose_RGB(RGB_Decomp* dcmp)
{
	bool result;
	...
	result = bfgs(rgb_decomp_obj_f, rgb_decomp_obj_Df, dcmp, dcmp->s, dcmp->dim, 1000);
...
bool test_rgb2rgbw()
{
	RGB base[] = {{0.9,0.1,0.05},{0.0,0.9,0.1},{0.05,0.01,1},{0.95,0.9,1}};
	RGB obj = {0.6,0.3,0.7};
